...
/**
 * Class DBCore
 * extends medoo v1.1.3
 * User: Yan<me@xiaoyan.me>
 * DateTime: 2016-11-27 21:49
 */
class DBCore
{
    // General
    ...
    public function __construct($options = null)
    {
        try {
            ...
            throw new DBException('Connect to database failed!', 10001);
        }
    }

    public function table_exists($tableName){
        $data = $this->fetch("SHOW TABLES LIKE '%{$tableName}%';");
        return $data;
    }
    ...
    public function orderBy($sort = null)
    {
        ...
        if (!isset($this->whereRules['ORDER'])) {
            if (is_array($sort)) $this->whereRules['ORDER'] = $sort;
            else $this->whereRules['ORDER'] = array($sort);
        } else {
            $this->whereRules['ORDER'][] = $sort;
        }
        return $this;
    }
    ...
    {
        if(null === $where) return $this->whereRules;
        if (is_string($where)) {
...
//            $fields = '`'.implode('`,`',explode(',',$fields)).'`';
    ...
    public function query($query)
    {
        ...
        try {
            ...
            if (strtolower($this->charset) == 'gbk' && YCFCore::getInstance()->getConf('charset') != 'gbk') {
...
//        return '"' . $this->prefix . $table . '"';
    ...
    protected function column_quote($string)
    {
        ...
        if (isset($column_match[2], $column_match[3])) {
            if(!$column_match[3]) $column_match[3] = '*';
...
//                    $where_clause .= ($where_clause != '' ? ' AND ' : ' WHERE ') . ' MATCH ("' . str_replace('.', '"."', implode($MATCH['columns'], '", "')) . '") AGAINST (' . $this->quote($MATCH['keyword']) . ')';
            ...
            if (isset($where['ORDER'])) {
                ...
                if (is_array($ORDER)) {
                    ...
                    foreach ($ORDER as $column => $value) {
                        if(strpos($value,' ') != -1){
                            list($column,$value) = explode(' ',$value);
                        }
                        if (is_array($value)) {
    ...
    protected function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
    {
        ...
        if ($join == null) $join = $this->join;
        if ($columns == null) $columns = $this->fields;
        if ($where == null) $where = $this->whereRules;
...
//                                    $this->table_quote(isset($match[5]) ? $match[5] : $match[3]) . '."' . $value . '"';
    ...
    public function select($table = null, $join = null, $columns = null, $where = null)
    {
        ...
        $columns = $where == null ? $join : $columns;
...
//        $query = $this->query($this->select_context($this->tableName, $this->join, $this->fields, $this->whereRules));
        ...
        $is_single_column = (is_string($columns) && $columns !== '*' && count(explode(',',$columns)) == 1);
        if ($is_single_column) {
