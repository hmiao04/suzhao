...
class RedisClient
{
    ...
    public function setValue($key, $value, $expire = 0)
    {
        ...
        if ($expire > 0) $this->cmd('EXPIRE', $key, $expire);
        return $this->set();
    }


    public function setExpireTime($key,$seconds){
        return $this->cmd('EXPIRE', $key, $seconds)->execSuccess();
    }

    public function exists($key)
    {
        return $this->cmd('EXISTS', $key)->execSuccess();
    }

    private function execSuccess(){
        $data = $this->set();
    ...
    {
        return $this->cmd('DEL', $key)->execSuccess();
    }

    public function ttl($key)
    {
        return $this->cmd('TTL', $key)->set();
        return $data && $data[0] == 1;
    }
    ...
    private function get($line = false)
    {
        if (!$this->handle) {
            return false;
        }
