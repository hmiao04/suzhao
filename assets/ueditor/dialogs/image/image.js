...
(function () {
    ...
    function initView(){
        var url = editor.getActionUrl('dirs'),
            ...
            'onsuccess': function (r) {
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        var p = document.querySelector('#picture-dirs');
                        json.list.forEach(function(o,index){
                            var item = document.createElement('a');
                            p.appendChild(item);
                            item.setAttribute('href','#');
                            item.setAttribute('data-cate-id',o['id']);
                            item.innerText = o['cate_name'];
                            item.addEventListener('click',function(){
                                if(this.className.indexOf('active') != -1){
                                    return false;
                                }
                                var ls = document.querySelectorAll('#picture-dirs>a');
    ...
    OnlineImage.prototype = {
        ...
        initData: function () {
            ...
            this.listIndex = 0;
            this.cateId = document.querySelector('#picture-dirs>a.active').getAttribute('data-cate-id');
            this.listEnd = false;
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    if(currentData['url']){
                        ...
                        img.width = 113;
                        img.setAttribute('src', urlPrefix + list[i].url + (list[i].url.indexOf('?') == -1 ? '_110x110q90.webp?resize/w=110/h=110/quality=100&noCache=':'&noCache=') + (+new Date()).toString(36) );
                        img.setAttribute('_src', urlPrefix + list[i].url);
                        this.list.insertBefore(item, this.clearFloat);
                    }else if(currentData['type'] == 'dir'){
